...
export const SharedScheduleSelectorWizard = React.memo<SharedScheduleSelectorWizardProps>(
  ({ agenda, onSuccess, className }) => {
    const { toast } = useToast();
    const [currentStep, setCurrentStep] = useState<Step>('whatsapp-validation');
    ...
    const handleWhatsAppValidated = useCallback(
      async (personId: string, userData?: ExistingUserFullData) => {
        ...
        setResponsavelWhatsapp(userData?.telefone ? Number(userData.telefone) : 0);
    ...
    }, [wizardData, agenda, responsavelId, responsavelWhatsapp, toast, onSuccess]);
        ...
        case 'whatsapp-validation':
          return (
            <WhatsAppValidationStep
              onContinue={handleAccessDenied}
              onExistingPersonContinue={handleWhatsAppValidated}
            />
          );
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => {
                    const paciente = pacientesDoResponsavel.find((p) => p.id === value);
                    setWizardData((prev) => ({
                      ...prev,
                      paciente_id: value,
                ...
                <div className="space-y-4">
                  {Object.entries(slotsPorData)
                    ...
                    .map(([dateKey, slots]) => {
                      ...
                      const diaFormatado = format(date, "EEEE, dd 'de' MMMM", {
                        locale: ptBR,
                      });
                        ...
                        <div key={dateKey} className="space-y-2">
                          ...
                          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                            {slots
                              ...
                              .map((slot) => {
                                const hora = format(parseISO(slot.data_hora), 'HH:mm');
                                const isSelected = wizardData.slot_id === slot.id;
              ...
              <Card className="bg-primary/5">
                ...
                <CardContent className="space-y-3 text-sm">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <span className="font-medium">{wizardData.paciente_nome}</span>
                  </div>

                  <div className="grid grid-cols-2 gap-2">
                    <span className="text-muted-foreground">Serviço:</span>
                    <span className="font-medium">{wizardData.tipo_servico_nome}</span>
                  </div>

                  {wizardData.local_nome && (
                    <div className="grid grid-cols-2 gap-2">
                      <span className="text-muted-foreground">Local:</span>
                      <span className="font-medium">{wizardData.local_nome}</span>
                    </div>
                  )}
                  ...
                  {wizardData.empresa_nome && (
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      <span className="font-medium">{wizardData.empresa_nome}</span>
                    </div>
                  )}
                  ...
                  {slotDate && (
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      <span className="font-medium">
                        {format(slotDate, "EEEE, dd/MM 'às' HH:mm", { locale: ptBR })}
                      </span>
                    </div>
                  )}
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <span className="font-medium">{agenda.profissional_nome}</span>
                  </div>
                </CardContent>
              </Card>
            </div>
    ...
    return (
      <div className={cn('w-full max-w-2xl mx-auto', className)}>
        {/* Header da Agenda */}
        {currentStep !== 'whatsapp-validation' && currentStep !== 'access-denied' && (
