...
export const EstagioCandidatoDetail: React.FC<EstagioCandidatoDetailProps> = ({
  ...
  const [entrevista, setEntrevista] = useState<EntrevistaFicha>({});
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      const updated = await updateCandidaturaAvaliacao(row.id, {
        ...
        entrevista: { ...entrevista, atualizado_em: new Date().toISOString() },
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[95vw] sm:max-w-3xl lg:max-w-5xl max-h-[90vh] overflow-y-auto overflow-x-hidden">
        <DialogHeader>
        ...
        <Tabs defaultValue="candidatura" className="w-full">
          ...
          >
            ...
            {/* Blocos principais: 2 colunas no desktop, empilha no mobile */}
            <div className="grid lg:grid-cols-2 gap-x-6 gap-y-4 items-start">
              ...
              {/* Coluna direita: escrita + estilo + avaliação */}
              <div className="space-y-4 min-w-0">
                ...
                <section className="space-y-2">
                  ...
                  <div className="rounded-xl border border-border/60 p-3">
                    <p className="text-xs font-medium text-muted-foreground mb-1">
                      Como explicaria a demora do tratamento para uma mãe
                      ansiosa?
                    </p>
                    <p className="text-sm text-foreground whitespace-pre-wrap break-words leading-relaxed">
