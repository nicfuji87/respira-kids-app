...
export const WeekEventBlock = React.memo<WeekEventBlockProps>(
  ({
    ...
    const calculatePosition = () => {
      ...
      // Calcula largura e posição horizontal para eventos sobrepostos
      const width =
        ...
        <div className="w-full h-full p-0.5">
          <EventCard
            ...
            className={cn(
              ...
              'overflow-hidden',
              // Estilos responsivos baseados na altura
              {
                'text-[10px]': position.height < 40,
                'text-xs': position.height >= 40 && position.height < 60,
                'text-sm': position.height >= 60,
              }
            )}
          />
        </div>
